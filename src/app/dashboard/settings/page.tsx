...
import { getMyLimits, getPublicPlans } from "@/lib/supabase/queries";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress";
import { AboutSections } from "@/components/shared/about-sections";
import { formatBytes } from "@/lib/utils/format";
import { Sparkles, Mail, Files, HardDrive, Link2, FileUp, AlertTriangle, Info } from "lucide-react";
...
export default async function SettingsPage() {
  ...
  const [{ data: profile }, limits, plans] = await Promise.all([
    ...
    <div className="space-y-6 p-4 lg:p-8">
      ...
      <div className="space-y-2 pt-4">
        <h2 className="flex items-center gap-2 text-xl font-semibold tracking-tight">
          ...
          Acerca de mi_IPFS_Drive
        </h2>
        <p className="text-sm text-muted-foreground">
          Qué es IPFS, características, planes y el stack tecnológico detrás de la aplicación.
        </p>
      </div>
